...
import { commentServiceClient, recordCommentWebhookEvent, upsertServerComments, type ServerCommentInput } from "@/lib/comment-server";
    ...
    for (const entry of payload.entry ?? []) {
      for (const change of entry.changes ?? []) {
        ...
        if (event.commentId && event.text) {
          ...
          const fetched = await fetchInstagramCommentById(connection.access_token, event.commentId, event.mediaId).catch(() => null);
          if (fetched) {
            comment = {
              source: "instagram",
